...
var _ = Describe("Locket / rds-metric-collector process group", func() {
	...
	It("Should start cleanly if it can acquire a lock", func() {
		...
		Eventually(mockLocketServerSession.Buffer, 5*time.Second).Should(gbytes.Say("grpc.grpc-server.started"))
		...
		Expect(err).ToNot(HaveOccurred())
		Eventually(rdsMetricCollectorSession.Buffer, 5*time.Second).Should(gbytes.Say("rds-metric-collector.locket-lock.acquired-lock"))
	...
	It("Should hang if it cannot acquire a lock", func() {
		...
		Eventually(rdsMetricCollectorSession.Buffer, 5*time.Second).Should(gbytes.Say("rds-metric-collector.locket-lock.failed-to-acquire-lock"))
	...
	It("Should crash if it loses the lock", func() {
		...
		Eventually(rdsMetricCollectorSession.Buffer, "5s").Should(gbytes.Say("rds-metric-collector.locket-lock.acquired-lock"))
